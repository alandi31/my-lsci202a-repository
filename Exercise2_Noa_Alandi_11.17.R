...
d %>% 
  ...
  labs(x="H statistic", y = "Mean RT",title="Mean RT and H-Statistic per Language")+
  theme_bw() +
...
d %>%
  ...
  ggplot(aes(x = name_H, y = RT_M)) +
  geom_density_2d() +
  labs(x="H statistic", y = "Mean RT",title="RT model predictions")+
  facet_wrap(~lang, nrow = 2) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+
...
d %>%
  ...
  facet_wrap(~lang, nrow=2) +
  labs(x="H statistic", y = "Mean RT",title="RT model residuals")+
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+
...
# and German is underestimated by the model. This indicates
...
d %>%
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+
  labs(x="H statistic", y = "Mean RT",title="RT by Lang predictions") +
  geom_line(aes(y=pred), 
            color="red",
            size = 1) +
...
d %>%
  ...
  ggplot(aes(x = name_H, resid)) +
  geom_point(alpha=1/8) +
  labs(x="H statistic", y = "Mean RT",title="RT by Lang residuals") +
  facet_wrap(~lang, nrow=2) +
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5))+
...
d %>%
  ...
  geom_density_2d() +
  facet_wrap(~lang, nrow=2) +
  labs(x="H statistic", y = "Mean RT",title="RT by Lang slope predictions") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+
  geom_line(aes(y=pred), 
            color="red",
...
d %>%
  ...
  geom_point(alpha=1/8) +
  facet_wrap(~lang, nrow=2) +
  labs(x="H statistic", y = "Mean RT",title="RT by Lang slope residuals") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+
...
d4 %>%
  ggplot(aes(fill = Response.variable, x = Animacy)) +
  geom_bar(position="fill") +
  facet_wrap(~Role) +
  scale_y_continuous(labels = scales::percent) +
  labs(y="Proportion", title = "Animacy by Role")+
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+
...
d2 %>%
  ...
  facet_wrap(~Role) +
  scale_y_continuous(labels = scales::percent) +
  labs(y="Proportion", title = "Definiteness by Role")+
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.position = "bottom")+
  theme()
...
#Empricial probabilites calculated according to model specification: Role*Animacy*Definiteness
...
#When calculating the empirial probabilities by pair (Role/Animacy Role/Definiteness) 
#we see that for Role/Animacy, the model is over-predicting the rate of double object 
#constructions for each combination, but more so for the Theme/Animate combination. 
#Yet when we consider the predictions side by side with the empirical proportions for 
#Role/Definiteness, the predictions are much better. They are still over-predicting, 
#but the difference is doesn't appear visually to be signifcant. 
